...
최근 Yellow Store 프로젝트를 진행하면서 DDD(Domain-Driven Design)를 공부하고 있습니다. DDD를 공부하다 보니 자연스럽게 MSA(Microservices Architecture)와의 연관관계에 대해 생각하게 되었습니다. 이번 포스트에서는 DDD와 MSA가 어떻게 연결되는지, 그리고 왜 DDD가 마이크로서비스로의 전환을 위한 좋은 기반이 되는지 살펴보겠습니다.
...
Yellow Store 프로젝트에서도 이 원칙을 따라 DDD를 공부하고 있으며, 향후 필요시 각 바운디드 컨텍스트를 독립적인 마이크로서비스로 분리할 수 있는 기반을 마련할 예정입니다.
